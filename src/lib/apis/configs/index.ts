...
export const getPromptOptimizerConfig = async (token: string) => {
        let error = null;

        const res = await fetch(`${WEBUI_API_BASE_URL}/configs/prompt_optimizer`, {
                method: 'GET',
                headers: {
                        'Content-Type': 'application/json',
                        Authorization: `Bearer ${token}`
...
export const setPromptOptimizerConfig = async (token: string, config: object) => {
        let error = null;
