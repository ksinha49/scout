...
| 2025-12-02 | X1BA          | CWE-400            | Sanitized the tag details before passign to the regular expression |                                                           
| 2025-12-02 | X1BA          | CWE-400            | Polynomial regular expression used on uncontrolled data |                                                           
"""
import time
import logging
import sys
        ...
        async def post_response_handler(response, events):
            ...
            def tag_content_handler(content_type, tags, content, content_blocks):
                ...
                def extract_attributes(tag_content):
                    """Extract attributes from a tag if they exist using html.parser (safer than regex)."""
                    ...
                    parser = AttrParser()
                    # Wrap in a dummy tag so the parser can process attributes
                    parser.feed(f"<t{tag_content}>")
                    return parser.attrs
