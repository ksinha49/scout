...
from open_webui.env import (
    ...
    OLLAMA_SERVER_CERT,  #MOD TAG AMER-ENH
    IS_CERT_REQ # MOD TAG AMER-ENH
)
...
async def send_post_request(
    ...
    try:
        ...
        detail = None
        ## MOD: CWE-209 Log the error and return a generic message
        ...
        if r is not None:
            try:
                ...
                log.exception(f"Error occurred in post_streaming_url:{e}")
...
async def get_ollama_tags(
    ...
    if url_idx is None:
        ...
        key = get_api_key(url_idx, url, request.app.state.config.OLLAMA_API_CONFIGS)
         # MOD TAG AMER-ENH resolved the certificate error
        cert_val =  OLLAMA_SERVER_CERT if IS_CERT_REQ else IS_CERT_REQ
        ...
        try:
            ...
            ## MOD: CWE-209 Log the error and return a generic message
            log.exception(f"Error fetching tags:{e}")
            ...
            if r is not None:
                try:
                    ...
                    detail = "Ollama: An error occurred, please try again."
    ...
    if request.app.state.config.ENABLE_OLLAMA_API:
        if url_idx is None:
            ...
            try:
                ...
                ## MOD: CWE-209 Log the error and return a generic message
                log.exception(f"Error fetching version information:{e}")
...
async def copy_model(
    ...
    try:
        ...
        ## MOD: CWE-209 Log the error and return a generic message
        log.exception(f"Error copying model:{e}")
        ...
        if r is not None:
            try:
                ...
                detail = "Ollama: An error occurred while copying the model"
...
async def delete_model(
    ...
    try:
        ...
        ## MOD: CWE-209 Log the error and return a generic message
        log.exception(f"Error in delete_model:{e}")
        ...
        if r is not None:
            try:
                ...
                detail = "Ollama: An error occurred while deleting the model"
...
async def show_model_info(
    ...
    try:
        ...
        ## MOD: CWE-209: Log the error internally and return a generic error to the client
        log.exception(F"Error in show_model_info:{e}")
        ...
        if r is not None:
            try:
                ...
                detail = "Ollama: An error occurred while retrieving model information."
...
async def embed(
    ...
    try:
        ...
        log.debug(f"generate_ollama_embeddings {data}")
        ...
        ## MOD: CWE-209: Log exception and return a generic message to the client
        log.exception(f"Error in generate_ollama_embeddings:{e}")
        detail = None
        if r is not None:
            try:
                res = r.json()
                ...
                detail = "Ollama: An error occurred during embedding generation"
...
async def embeddings(
	...
	##CWE-918 Added Ollama Server cert for Ollama server cert verify
    try:
        r = requests.request(
            method="POST",
        ...
        if r is not None:
            try:
                ...
                detail = "Ollama: An error occurred during batch embedding generation"
...
async def get_openai_models(
    ...
    if url_idx is None:
        ...
        try:
            ...
            ## MOD: CWE-209: Log exception and return a generic message to the client
            log.exception(f"Error in get_openai_models{e}")
            error_detail = "AmeritasAI: Server Connection Error"
            if r is not None:
                try:
                    res = r.json()
                    if "error" in res:
                    ...
                    error_detail = "Ollama: An error occurred while retrieving models"
...
async def download_model(
    ...
    ## MOD: CWE-918 Full server-side request forgery fix
    ...
    log.info(f"Downloading from: {hostname}")
    ...
    if file_name:
       ...
       ## MOD: CWE-209: Log the error internally and send a generic response to the client
       #return StreamingResponse(
       ...
       #)
        ...
        try:
            return StreamingResponse(
                download_file_stream(url, parsed_url, file_path, file_name),
            )
        except Exception as e:            
            ## MOD: CWE-209 Log error and return a generic response to the client         
            log.exception("Error in download_model")  # Internal logging
            raise HTTPException(
                status_code=500,
                detail="An error occurred while downloading the model. Please try again later."
            )
    else:
        return None
