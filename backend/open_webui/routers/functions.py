...
|------------|----------------|--------------------|-----------------------------------------------------------------------------------------------------|
| 2024-11-05 | AAK7S          | CWE-22             | Updated `create_new_function` function to sanitize user-provided `form_data.id` input.               |
|            |                |                    | Applied `secure_filename` to prevent path traversal attacks when creating the function cache dir.    |
"""
...
def is_path_within_directory(base_dir, target_path):
    # Ensure the target path is within the base directory
    return Path(os.path.commonpath([base_dir, target_path])) == base_dir
############################
# GetFunctions
############################
...
async def create_new_function(
    ...
    ## MOD: CWE-22: Sanitize form_data.id to prevent path traversal
    ...
    if function is None:
        try:
            ...
            ## MOD: CWE-22: Create a secure path for the function cache directory
            #function_cache_dir = Path(CACHE_DIR) / "functions" / form_data.id
            #function_cache_dir.mkdir(parents=True, exist_ok=True)
            function_cache_dir = Path(CACHE_DIR) / "functions" / secure_filename(form_data.id.lower())
            secure_function_cache_dir = function_cache_dir.resolve()

            # Ensure the cache directory is within the allowed directory
            if not is_path_within_directory(Path(CACHE_DIR).resolve() / "functions", secure_function_cache_dir):
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Path traversal detected in function cache directory"
                )

            secure_function_cache_dir.mkdir(parents=True, exist_ok=True)
