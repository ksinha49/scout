...
|------------|----------------|--------------------|-----------------------------------------------------------------------------------------------------|
| 2024-11-04 | AAK7S          | CWE-22             | Updated `get_model_path` function to mitigate path injection vulnerability.                         |
|            |                |                    | Added path sanitization and validation using `secure_filename` and `os.path.normpath`.              |
|            |                |                    | **Old Code:** Directly using the `model` path, which could lead to path traversal risks.            |
|            |                |                    | **New Code:** Sanitize the `model` path, normalize it, and validate that it does not escape the     |
|            |                |                    | intended directory.                                                                                 |
...
def get_model_path(model: str, update_model: bool = False):
    log.debug(f"Original model: {model}")
    ## MOD: CWE-22:Use a secure filename to prevent path injection attacks
    if "/" in model:
        base, model_name = model.split("/", 1)
        sanitized_model_name = secure_filename(model_name)
        model = f"{base}/{sanitized_model_name}"
    else:
        model = secure_filename(model)
    ## MOD: CWE-22:End of change
    ...
    ## MOD: CWE-22 Modification Start
    # Normalize the path and ensure it does not escape `cache_dir`
    normalized_path = os.path.normpath(os.path.join(cache_dir, model))
    if not normalized_path.startswith(cache_dir):
        raise ValueError("Invalid model path: potential path traversal detected.")
