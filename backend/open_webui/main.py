...
https://github.com/ameritascorp/chatAmeritas-ui
"""
)
...
app.state.config.MODEL_FALLBACK_PRIORITIES = MODEL_FALLBACK_PRIORITIES
...
if app.state.config.STT_ENGINE == "":
    log.info(
        f"Preloading Whisper model '{WHISPER_MODEL.value}' into {WHISPER_MODEL_DIR}"
    )
    try:
        app.state.faster_whisper_model = audio.set_faster_whisper_model(
            WHISPER_MODEL.value, auto_update=WHISPER_MODEL_AUTO_UPDATE
        )
        ...
        log.warning(f"Failed to preload Whisper model '{WHISPER_MODEL.value}': {e}")
else:
...
app.state.config.ENABLE_PROMPT_OPTIMIZER = ENABLE_PROMPT_OPTIMIZER
...
app.state.config.PROMPT_OPTIMIZER_MODEL = PROMPT_OPTIMIZER_MODEL
app.state.config.PROMPT_OPTIMIZER_SYSTEM_PROMPT = PROMPT_OPTIMIZER_SYSTEM_PROMPT
app.state.config.AUTOCOMPLETE_GENERATION_PROMPT_TEMPLATE = (
    AUTOCOMPLETE_GENERATION_PROMPT_TEMPLATE
)
app.state.config.AUTOCOMPLETE_GENERATION_MODEL = (
    AUTOCOMPLETE_GENERATION_MODEL
)
...
app.include_router(prompt_optimizer.router, prefix="/api/v1/prompt-optimizer", tags=["prompt-optimizer"])
    ...
    return {
        **({"onboarding": True} if onboarding else {}),
        ...
        "features": {
            ...
            "enable_prompt_optimizer": app.state.config.ENABLE_PROMPT_OPTIMIZER,
            **(
                {
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
            async with session.get(
                "https://api.github.com/repos/ameritascorp/chatAmeritas-ui/releases/latest"
            ) as response:
...
@app.get("/oauth/{provider}/silent-login")
async def oauth_silent_login(provider: str, request: Request):
    return await oauth_manager.handle_login(request, provider, prompt="none")
...
async def get_manifest_json():
    return {
        ...
        "description": "Scout is Ameritas' GenAI chat interface solution.",
        "start_url": "/",
        "display": "standalone",
