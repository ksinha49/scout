print("Topic Modeling Script Begin")
...
csv_file = sys.argv[1]
#csv_file = "../data/historical_data_20250312_165107.csv"
...
topic_model = BERTopic(language='english', nr_topics=10) #, calculate_probabilities=True, verbose=True)
topics, probs = topic_model.fit_transform(df['human_processed'])
...
if not os.path.exists(full_path):
    ...
    print("topic_models subdirectory created")
...
df['Topic'] = topic_model.get_document_info(df['human_processed'])['Topic']
df['Topic_Name'] = topic_model.get_document_info(df['human_processed'])['CustomName']
df['Top_Words_BERTopic'] = topic_model.get_document_info(df['human_processed'])['Top_n_words']
...
print("Top Words Per Chat Complete")
...
df_write = df[['chat_id', 'user_id', 'email', 'title', 'chat_created_at', 'chat_updated_at', 'conversation', 'num_thumbs_up', 'num_thumbs_down', 'D1', 'D2', 'chat_sentiment', 'Topic', 'Topic_Name', 'Top_Words_BERTopic', 'Top_Words_Count', 'Top_Words_TFIDF']]
